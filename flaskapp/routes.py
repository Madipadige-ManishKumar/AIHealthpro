...
def Heartsub():
        ...
        if flag==1:
            ...
            X = [[item.Age, item.Gender, item.family_history, item.HyperTension, item.smoking, 
                    item.stress, item.alcoholic,item.BodyWeight,item.Excessive_intakeof_salt, item.Excessive_intakeof_coffee] for item in data]
...
def Kidneysub():
    ...
    if flag==1:
        ...
        data=kidney.query.all()
        X = [[item.Age, item.FamilyHistory, item.PhysicalExcerise, item.obesity, item.Hypertension, 
                item.HeartDieases, item.Smoking,item.excessive_painkillers,item.Alcohol, item.diabetes] for item in data]
...
def Liversub():
    ...
    if flag ==1:
        ...
        data=liver.query.all()
        X = [[item.Gender, item.Family_History, item.Alcohol, item.smoking, item.Sleep_disorder, 
                item.obesity, item.excessive_medication,item.excessive_painkillers,item.diabetes] for item in data]
...
def lungssub():
            ...
            X = [[item.Age, item.family_history, item.smoking, item.allergies, item.weight, 
                item.Exercise, item.Location] for item in data]
            y = [item.result for item in data]
